...
namespace Strava
{
	namespace Async
	{
		...
		{
		public:
			std::string GetServerInterface() const override;
			...
			void SetTokenExchangeTarget(const std::string& tokenExchangeTarget) override;
		...
		private:
			ServerNetworkParametersBundle(const std::string& serverInterface, uint16_t serverPort, const std::string& certFile, const std::string& pkFile, const std::string& pkPassphrase);
